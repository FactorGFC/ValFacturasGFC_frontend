...
@Component({
  ...
  templateUrl: './actualizacuentacontable.component.html',
  styles: []
})
export class ActualizaCuentaContableComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    Swal.showLoading();
      ...
      this._contabilidad.getCuentaContable( this.ido ).subscribe( resp => { this.cuenta = resp; Swal.close(); }
      ,(err) => {Swal.fire({title: 'Ocurrio un error al cargar la informacion', allowOutsideClick: false })} );

  }


  actualizaCuentaContable() {
    ...
    this._contabilidad.actualizaCuentaContable( this.ido, params).subscribe( () => {
    Swal.fire({
      title: 'Modificacion de cuenta contable exitosa',
      text: '',
      icon: 'success',
