...
export class CreaCreditlineComponent implements OnInit {
 ...
 // debtype: any[] = [];
  ...
  ngOnInit() {
    ...
    /*  this._contribuyentesService.getFundingCreditLineType().subscribe(resp => {console.log(resp)
      if ( resp[0] === 'FONDEADOR' || resp[0].length === 0 ) {
      console.log('el parametro es fondeador o no existe');
      this.fondeador = true;
      } else if ( resp[0] === 'CADENA' ) {
      console.log('el parametro es cadena o no existe');
      this.fondeador = false;
      this._contribuyentesService.getCadenasParaFondeador().subscribe( resp => {this.cadenasfondeadores = resp
      console.log(this.cadenasfondeadores);
      } )
      this._contribuyentesService.getDebType().subscribe(resp => {this.debtype = resp; console.log(this.debtype)})
      } else {
      console.log('no cayo por lo tanto fondeador');
      this.fondeador = true;
      }
      }) */
