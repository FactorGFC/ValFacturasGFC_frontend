...
export class SolicitudDeFondeoComponent implements OnInit {
  ...
  this.cadenaso.sort();
    ...
    this._fundersservice.getfundersfinancial().subscribe( resp => {this.financieras = resp;} );
 ...
 swal2.fire({
  ...
  /*  workbook.xlsx.writeBuffer().then((data) => {
    ...
    this._firestorage.upload(filepath, file).then(() => { fileRef.getDownloadURL().subscribe( resp => { console.log(this.uploadURL);
      this.uploadURL = resp;
      const params = {
      token: '',
      secret_key: '',
      ...
      this._fundersservice.agregaattached( ids, params ).subscribe( () => this._fundersservice.getEnviaMail( ids ).subscribe( resp => console.log(resp) )
      ,  (err) => {
        console.log(err);
      } );
      } ); });
