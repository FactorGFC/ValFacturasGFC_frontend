  ...
  ngOnInit() {
   ...
   // this.getAcceso('/reportesolicitudes');
    ...
    //  { field:  'id_factura', header: 'ID'},
      ...
      { field:  'cesion_derechos', header: 'Cesion derechos'},
      { field:  'firma_digital', header: 'Firma digital'},
      { field:  'xml_firma_digital', header: 'XML firma digital'}
  ];
...
set selectedColumns(val: any[]) {
  ...
  exportexcel() {
    ...
    let pos = 3;
     ...
     const wj: XLSX.WorkSheet = XLSX.utils.sheet_to_json(ws);
     for (const prop in wj) {
        ws[`S${pos}`].l = { Target:ws[`S${pos}`].v, Tooltip:"" };
        ws[`T${pos}`].l = { Target:ws[`T${pos}`].v, Tooltip:"" };
        ws[`U${pos}`].l = { Target:ws[`U${pos}`].v, Tooltip:"" };
        pos = pos+1;
     }
    // ws['S3'].l = { Target:ws['S3'].v, Tooltip:"Find us @ SheetJS.com!" };
